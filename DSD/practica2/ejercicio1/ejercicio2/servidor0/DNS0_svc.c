...
static void
dns0_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union {
...
main (int argc, char **argv)
{
	...
	if (!svc_register(transp, DNS0, DNSVERS, dns0_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (DNS0, DNSVERS, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, DNS0, DNSVERS, dns0_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (DNS0, DNSVERS, tcp).");
		exit(1);
	}
