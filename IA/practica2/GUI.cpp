...
// GLfloat anguloCuboX = 92.0f;
// GLfloat anguloCuboY = 180.0f;
// GLfloat anguloCuboZ = -270.0f;

GLfloat anguloCuboX = 0;
...
GLfloat anguloPespectiva =22.0f;
...
GLUI_StaticText *mensaje_text,*total_score_text,*average_score_text, *juega_txt;
    ...
    if (agent!=NULL and env!=NULL){
	    ...
	    switch(orienta){
	    	case 0: fila--;
	    			for (int i=0; i<9; i++)
	    				for (int j=0; j<5; j++)
	    					if (fila-i<0 or j-2+columna<0 or j-2+columna>199){
	    						vision[i][j]='?';
	    						objetos[i][j]='?';
	    					}
	    					else{
	    						vision[i][j]=agent->mapa_entorno_[fila-i][j-2+columna];
	    						objetos[i][j]=agent->mapa_objetos_[fila-i][j-2+columna];
	    					}
	    			break;
	    	...
	    	case 1: columna++;
	    			for (int j=0; j<9; j++)
	    				for (int i=0; i<5; i++)
	    					if (columna+j>199 or i-2+fila<0 or i-2+fila>199){
	    						vision[j][i]='?';
	    						objetos[i][j]='?';
	    					}
	    					else{
	    						vision[j][i]=agent->mapa_entorno_[i-2+fila][columna+j];
	    						objetos[j][i]=agent->mapa_objetos_[i-2+fila][columna+j];
	    					}
	    			break;
	    	case 2: fila++;
	    			for (int i=0; i<9; i++)
	    				for (int j=0; j<5; j++)
	    					if (fila+i>199 or 2-j+columna<0 or 2-j+columna>199){
	    						vision[i][j]='?';
	    						objetos[i][j]='?';
	    					}
	    					else{
	    						vision[i][j]=agent->mapa_entorno_[fila+i][2-j+columna];
	    						objetos[i][j]=agent->mapa_objetos_[fila+i][2-j+columna];
	    					}
	    			break;
	    	case 3: columna--;
	    			for (int j=0; j<9; j++)
	    				for (int i=0; i<5; i++)
	    					if (columna-j<0 or 2-i+fila<0 or 2-i+fila>199){
	    						vision[j][i]='?';
	    						objetos[i][j]='?';
	    					}
	    					else{
	    						vision[j][i]=agent->mapa_entorno_[2-i+fila][columna-j];
	    						objetos[j][i]=agent->mapa_objetos_[2-i+fila][columna-j];
	    					}
	    			break;
	    }
	    ...
	    for (int i=0; i<=8; i++){
	        for (int j=-2; j<=2; j++){
	            switch(vision[8-i][j+2]){
	              case 'P': glColor4f(0.0,0.0,0.0,1);
	                        drawCube(j,0, i,1,0.1);
	                    break;
	              case 'A': glColor4f(0.0,0.0,1.0,1);
	                        drawCube(j,0, i,1,0.0);
	                    break;
	              case 'S': glColor4f(0.6,0.6,0.6,1);
	                        drawCube(j,0, i,1,0.1);
	                    break;
	              case 'T': glColor4f(0.3,0.25,0.2,1);
	                        drawCube(j,0, i,1,0.1);
	                    break;
	              case 'K': glColor4f(1.0,1.0,0.0,1);
	                        drawCube(j,0, i,1,0.2);
	                    break;
	              case 'Z': glColor4f(0.3,0.3,0.3,1);
	                        drawCube(j,0, i,1,0.1);
	                    break;
	              case 'B': 
	                        drawTree(j,0, i,1,1);
	                    break;
	              case 'M': //glColor4f(0.6,0.0,0.0,1);
	                        //drawCube(j,0, i,1,1);
	                        drawWall(j,0,i,1,1);
	                    break;

	              case 'D': glColor4f(1.0,1.0,1.0,1);
	                        drawCube(j,0, i,1,1);
	              case '?': glColor4f(1.0,1.0,1.0,1);
	                        drawCube(j,0, i,1,0.1);
	                    break;
